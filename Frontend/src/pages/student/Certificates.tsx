...
import { useParams } from 'react-router-dom';  // Link được sử dụng cho các liên kết
import logoutIcon from "../../assets/images/elementDatabaseWeb4.png";  // Biểu tượng đăng xuất

const CertificatesPage: React.FC = () => {
  const { studentId } = useParams<{ studentId: string }>();
  const [certificates, setCertificates] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [openMenu, setOpenMenu] = useState(false);  // Kiểm soát trạng thái menu overlay

  useEffect(() => {
    const fetchCertificates = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/students/${studentId}/certificates`);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h1 style={{ fontSize: '28px', fontWeight: 800, color: '#E84040', marginLeft: '20px' }}>Chứng Chỉ</h1>
      </div>
